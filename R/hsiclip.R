#' Convert a hdf5 file containing hyperspectral imagery into a topographically and brdf corrected raster stack.
#'
...
#' @param raster.res resolution of output raster
#' @return A raster stack of topographic and brdf corrected reflectance data
#' @export

hsi.clip <- function(hy.file, ndvi.mask, brightness.mask, band.combo, 
                     ...
                     li, raster.res, shp.file.loc, shp.file.name){
  ...
  for (q in band.combo) {
    ...
    # lets turn this into a 10x10 raster
    fl.raster <- aggregate(x = refl.raster,
                           fact = raster.res,
                           fun = mean,
                           na.rm = TRUE)
    
    # crop the raster down
    rst.cr <- raster::crop(fl.raster, crop.shp)
