#' Convert a hdf5 file containing hyperspectral imagery into a topographically and brdf corrected raster based on supplied PLSR coefficients.
#'
...
#' 
#' Last, the PLSR coefficients are applied to each band of this topographic and brdf corrected imagery 
#' and then converted to a raster
#'
...
#' @param plsr.csv csv containing PLSR coefficients and intercept
#' @return A raster of forest functional traits
#' @export

hsi.plsr <- function(hy.file, ndvi.mask, brightness.mask, 
  ...
  for (q in plsr.index) {
    ...
    plsr.raster <- refl.raster * wl.coef
    
    if (s == 1){
      # stack this raster
      hsi.stack <- stack(hsi.stack, plsr.raster)
      hsi.stack <- hsi.stack + plsr.inter
    } else {
      hsi.stack <- hsi.stack + plsr.raster
    }
